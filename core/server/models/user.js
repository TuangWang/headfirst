...
User = hfBookshelf.Model.extend({

  tableName: 'users',

  defaults: function defaults() {
    var baseDefaults = hfBookshelf.Model.prototype.defaults.call(this);
  ...
  onSaving: function onSaving(newPage, attr, options) {
    ...
    if (this.hasChanged('slug') || !this.get('slug')) {
      tasks.slug = (function generateSlug() {
        return hfBookshelf.Model.generateSlug(
          User,
  ...
  onValidate: function validate() {
    ...
    userData = hfBookshelf.Model.prototype.toJSON.call(this);
  ...
  format: function format(options) {
    ...
    return hfBookshelf.Model.prototype.format.call(this, options);
  },
   ...
   *
   * @extends hfBookshelf.Model.findOne to include roles
   ...
   *
   * @extends hfBookshelf.Model.edit to handle returning the full object
  ...
  edit: function edit(data, options) {
    ...
    ops.push(function update() {
      return hfBookshelf.Model.edit.call(self, data, options).then(function then(user) {
        var roleId;
        ...
        return user.roles().fetch().then(function then(roles) {
          ...
          return hfBookshelf.model('Role').findOne({id: roleId});
   ...
   * @extends hfBookshelf.Model.add to manage all aspects of user signup
  ...
  add: function add(dataToClone, options) {
    ...
    function getAuthorRole() {
      return hfBookshelf.model('Role').findOne({name: 'Author'}, _.pick(options, 'transacting'))
    ...
    return hfBookshelf.Model.add.call(self, userData, options)
      ...
      .then(function (_roles) {
        ...
        if (_.isString(roles[0]) && !ObjectId.isValid(roles[0])) {
          return Promise.map(roles, function (roleName) {
            return hfBookshelf.model('Role').findOne({
              name: roleName
            }, options);
  ...
  transferOwnership: function transferOwnership(object, options) {
    ...
    return Promise.join(hfBookshelf.model('Role').findOne({name: 'Owner'}),
      ...
      .then(function then(results) {
        ...
        return Promise.join(hfBookshelf.model('Role').findOne({name: 'Administrator'}),
...
Users = hfBookshelf.Collection.extend({
  model: User
});

module.exports = {
  User: hfBookshelf.model('User', User),
  Users: hfBookshelf.collection('Users', Users)
};
